function  bbox = fcn_tracker(in_list, outfile)
...
%% parsing input list
gt1=in_list{1};
images=in_list(2:end-1);
gpu_id = in_list{end};
im1_name = images{im1_id};
im1 = double(imread(im1_name));
scale_factor = double(size(im1, 2)) / resize_width;
im1 = imresize(im1, [NaN, resize_width]);
if size(im1,3)~=3
    im1(:,:,2) = im1(:,:,1);
    im1(:,:,3) = im1(:,:,1);
end
...
% map ground truth
gt1 = 1 + round((gt1 - 1) / scale_factor);
gt1(3) = gt1(3) - gt1(1);
gt1(4) = gt1(4) - gt1(2);
GT = repmat(gt1, length(images), 1);

ch_num=512;
pf_param = struct('affsig', [10,10,.004,.00,0.00,0], 'p_sz', 64,...
            'p_num', 600, 'mv_thr', 0.1, 'up_thr', 0.15, 'pos_thr', 0.3, 'roi_scale', 2); % roi_scale = 2;
...
for im2_id = im1_id:fnum
    ...
    im2_name = images{im2_id};
    im2 = double(imread(im2_name));
    im2 = imresize(im2, [NaN, resize_width]);
    if size(im2,3)~=3
        im2(:,:,2) = im2(:,:,1);
        im2(:,:,3) = im2(:,:,1);
    end
    ...
    g_pre_map = permute(g_pre_map{1}, [2,1,3]);
...
%     base_location_l = [pos(1:2), location([3,4])];
    ...
    if max_scale_respones_l >= max_scale_respones_g %|| 1
        ...
        pos = [l_x, l_y];
        final_conf = max(l_pre_map(:));
    else
        recovered_scale = find(scale_response_g == max_scale_respones_g, 1);
        pos = [g_x, g_y];
        final_conf = max(g_pre_map(:));
    end
    confs(im2_id) = final_conf;
    ...
    % early stopping
    if final_conf < pf_param.mv_thr
        fprintf('Low confidence: early stopping.\n');
        break;
    end
end

bbox=positions(1:im2_id, :);
bbox(:,3) = bbox(:,1) + bbox(:,3);
bbox(:,4) = bbox(:,2) + bbox(:,4);
% map bbox back
bbox = round(1 + (bbox - 1) * scale_factor);
bbox = [bbox, confs(1:im2_id)];
if nargin > 1
    save(outfile, 'bbox');
end
end
...
elseif strcmp(type, 'trans_gaussian')
    sz = location([4,3]);
    output_sigma_factor = 1/12; %1/32;%1/16;
